...
class ShowControllerTest {
	...
	void 뮤지컬_상세_조회_성공() throws Exception {
		...
		given(showService.getDetail(anyLong(), nullable(Long.class))).willReturn(response);
	...
	void 공연_상세_조회_공지없음_빈목록_성공() throws Exception {
		...
		given(showService.getDetail(org.mockito.ArgumentMatchers.eq(10L), nullable(Long.class))).willReturn(response);
	...
	void 뮤지컬_상세_조회_실패() throws Exception {
		willThrow(new CustomException(ErrorCode.NOT_FOUND_SHOW))
			.given(showService).getDetail(org.mockito.ArgumentMatchers.eq(999L), nullable(Long.class));
